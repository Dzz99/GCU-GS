...
def process_image(image_path, resolution, ncc_scale, white_background):
    ...
    if ext == '.png':
        ...
        loaded_mask = torch.from_numpy(mask).float().unsqueeze(0)
...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
                 ...
                 ncc_scale=1.0,
                 preload_img=True, data_device = "cuda", white_background = False
                 ):
        ...
        if self.preload_img:
            gt_image, gray_image, loaded_mask = process_image(self.image_path, self.resolution, ncc_scale, white_background)
