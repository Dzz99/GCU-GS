...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background, 
    ...
    # 新增：保存 PGSR raw plane_depth
    render_depth_raw_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_depth_raw")
    render_normal_raw_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_normal_raw")

    makedirs(gts_path, exist_ok=True)
    makedirs(render_path, exist_ok=True)
    makedirs(render_depth_path, exist_ok=True)
    makedirs(render_normal_path, exist_ok=True)
    makedirs(render_depth_raw_path, exist_ok=True)
    makedirs(render_normal_raw_path, exist_ok=True)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        # 新增：保存 raw float depth，后续定量评估用这个
        depth_raw = depth.detach().cpu().numpy().astype(np.float32)
        np.save(os.path.join(render_depth_raw_path, view.image_name + ".npy"), depth_raw)

        # 原有：仅用于可视化的 depth jpg
        depth = depth.detach().cpu().numpy()
        depth_i = (depth - depth.min()) / (depth.max() - depth.min() + 1e-20)
        depth_i = (depth_i * 255).clip(0, 255).astype(np.uint8)
        depth_color = cv2.applyColorMap(depth_i, cv2.COLORMAP_JET)
        ...
        np.save(os.path.join(render_normal_raw_path, view.image_name + ".npy"), normal)        
        normal_vis = ((normal + 1) * 127.5).astype(np.uint8).clip(0, 255)
        PILImage.fromarray(normal_vis).save(os.path.join(render_normal_path, view.image_name + ".png"))
        if name == 'test':
            torchvision.utils.save_image(gt.clamp(0.0, 1.0), os.path.join(gts_path, view.image_name + ".png"))
            torchvision.utils.save_image(rendering, os.path.join(render_path, view.image_name + ".png"))
        else:
